<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Ubah Portofolio: ') }} {{ $portfolio->title }}
        </h2>
    </x-slot>

    <div class="py-12 bg-gray-50 min-h-[calc(100vh-64px)]">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('admin.portfolios.update', $portfolio) }}" method="POST" enctype="multipart/form-data" class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-100 p-8 space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label for="title" class="block text-sm font-semibold text-gray-700 mb-2">Nama Brand / Karya</label>
                    <input type="text" name="title" id="title" value="{{ old('title', $portfolio->title) }}" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="Contoh: Kopi Merapi Jiwa" required>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="category" id="category" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            ...
                            <option value="Custom" {{ old('category', $portfolio->category) == 'Custom' ? 'selected' : '' }}>Custom</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="order" id="order" value="{{ old('order', $portfolio->order) }}" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="Contoh: Desain kemasan pouch kopi roast bean kekinian." required>{{ old('description', $portfolio->description) }}</textarea>
                    @error('description') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                </div>

                <div>
                    <label for="image_file" class="block text-sm font-semibold text-gray-700 mb-2">Upload File Gambar Baru</label>
                    ...
                    <span class="text-xs text-gray-400 mt-1 block">Biarkan kosong jika tidak ingin mengubah file gambar.</span>
                    @error('image_file') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                </div>

                <div>
                    <label for="image_url" class="block text-sm font-semibold text-gray-700 mb-2">Atau Gunakan URL Gambar Baru</label>
                    <input type="url" name="image_url" id="image_url" value="{{ old('image_url', filter_var($portfolio->image, FILTER_VALIDATE_URL) ? $portfolio->image : '') }}" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="https://images.unsplash.com/photo-...">
                ...
                @if($portfolio->image)
                    <div>
                        ...
                        <img src="{{ $portfolio->image }}" alt="Portfolio Current" class="w-full h-48 rounded-2xl object-cover border border-gray-200 shadow-sm">
                    </div>
                @endif
